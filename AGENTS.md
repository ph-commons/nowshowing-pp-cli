# NowShowing Printed CLI Agent Guide

This directory is a generated `nowshowing-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
nowshowing-pp-cli doctor --json
nowshowing-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
nowshowing-pp-cli which "<capability>" --json
nowshowing-pp-cli <command> --help
```
...
```bash
nowshowing-pp-cli <command> --agent
```
...
```bash
nowshowing-pp-cli <command> --help
nowshowing-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `nowshowing-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `NOWSHOWING_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
