...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NOWSHOWING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NOWSHOWING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nowshowing-pp-cli/feedback")
...
When ` + "`NOWSHOWING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NOWSHOWING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nowshowing-pp-cli feedback list
  nowshowing-pp-cli feedback list --limit 5
  nowshowing-pp-cli feedback list --json`,
