...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"theater": {
			{Canonical: "sm-megamall", Values: []string{"sm megamall", "megamall", "sm city megamall"}},
			{Canonical: "sm-city-north-edsa", Values: []string{"sm north edsa", "north edsa", "sm city north edsa"}},
			{Canonical: "the-podium", Values: []string{"podium", "the podium"}},
			{Canonical: "power-plant-mall", Values: []string{"power plant", "powerplant", "rockwell"}},
			{Canonical: "greenbelt-3", Values: []string{"greenbelt", "greenbelt 3", "ayala greenbelt"}},
			{Canonical: "glorietta-4", Values: []string{"glorietta", "glorietta 4"}},
			{Canonical: "trinoma-mall", Values: []string{"trinoma", "trinoma mall"}},
			{Canonical: "up-town-center", Values: []string{"up town center", "up town", "u.p. town center"}},
			{Canonical: "robinsons-galleria-ortigas", Values: []string{"galleria", "robinsons galleria", "robinsons galleria ortigas"}},
			{Canonical: "robinsons-place-manila", Values: []string{"robinsons manila", "robinsons place manila"}},
			{Canonical: "ortigas-cinemas-estancia", Values: []string{"estancia", "ortigas cinemas estancia"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		dbPath := defaultDBPath("nowshowing-pp-cli")
		s, err := store.OpenWithContext(ctx, dbPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: nowshowing-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: nowshowing-pp-cli: learn init: %v\n", err)
		}
	})
}
