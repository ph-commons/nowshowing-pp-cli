...
// nowshowing-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "NOWSHOWING_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("nowshowing-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  nowshowing-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  nowshowing-pp-cli recall "<question>" --agent
  nowshowing-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nowshowing-pp-cli learnings list --agent
  nowshowing-pp-cli learnings list --query "<substring>"
  nowshowing-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  nowshowing-pp-cli learnings forget "<question>" --resource <id>
  nowshowing-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  nowshowing-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  nowshowing-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
