...
type rootFlags struct {
	...
	// invocation. Mirrors the NOWSHOWING_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "nowshowing-pp-cli",
		Short: `Nowshowing CLI — Metro Manila movie showtimes as a CLI — every tracked cinema in one now-playing board, cross-checked across two sources.`,
		Long: `Nowshowing CLI — Metro Manila movie showtimes as a CLI — every tracked cinema in one now-playing board, cross-checked across two sources.
...
Health check: run 'nowshowing-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("nowshowing-pp-cli {{ .Version }}\n")
	...
	rootCmd.AddCommand(newNovelMoviesCmd(flags))
	rootCmd.AddCommand(newNovelNowPlayingCmd(flags))
	rootCmd.AddCommand(newNovelSearchCmd(flags))
	rootCmd.AddCommand(newNovelTheatersCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newTheaterPromotedCmd(flags))
