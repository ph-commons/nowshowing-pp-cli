...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nowshowing-pp-cli doctor
  nowshowing-pp-cli doctor --json
  nowshowing-pp-cli doctor --fail-on warn
  nowshowing-pp-cli doctor --fail-on stale`,
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "nowshowing-pp-cli", suggestion)
						} else {
